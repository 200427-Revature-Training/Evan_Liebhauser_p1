...
export function saveReimb(reimb: Reimb): Promise<Reimb> {
    const sql = `INSERT INTO p1.ERS_REIMBURSEMENTS (REIMB_AMOUNT, REIMB_SUBMITTED. REIMB_DESCRIPTION, \
...
export function patchReimb(reimb: Reimb): Promise<Reimb> {
    ...
    const timeSubmitted = reimb.REIMB_SUBMITTED && reimb.REIMB_SUBMITTED.toISOString();
    const timeResolved = reimb.REIMB_RESOLVED && reimb.REIMB_RESOLVED.toISOString();
